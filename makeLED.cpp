...
int main(int argc, char* argv[]){
   if(argc!=2){
	cout << "Usage is makeLED and one of: " << endl;
        cout << "   on, off, flash, status, or toggle." << endl;
   ...
   if(cmd=="on"){
	...
	std::cout << "Toggle Selected" << std::endl;
	std::fstream fs;
	fs.open(LED3_PATH "/brightness", std::fstream::in);
	std::string line = "";
	// I took out the while loop. It was overwriting line with a blank line. I only need the first line anyways
	getline(fs, line);
	fs.close();
	std::cout << line << std::endl;
	if (line == "1") {
		std::cout << "LED is on. Turning off\n";
		removeTrigger();
		writeLED("/brightness", "0");
	}
	else if (line == "0") {
		std::cout << "LED is off. Turning on\n";
		removeTrigger();
		writeLED("/brightness", "1");
	}
	else {
